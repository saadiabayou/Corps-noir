...
def resolve_Wien():
    ...
    d=int(input(" \nSaisir 1 pour la température ou saisir 2 pour la longueur d'onde : "))
    ...
    if d==num_param[0]:
        ...
        lambda_max_m=(Wien_lambda(T))
        print("\nLa longeur d'onde maximale émise est : lambda_max=",lambda_max_m, "metre\n")
        
        lambda_max_nm=(convert_m_nm (lambda_max_m))
        ...
        if u_l=="nm":
            ...
            lambda_max_m=convert_nm_m(lambda_max)
            print("lambda_max_m =",lambda_max_m, "mètre")
        ...
        T_K=(Wien_T(lambda_max_m))
        print("\n\nLa température en Kelvin est : \n \nT = ",T_K, "K\n")
            
        T_deg=(convert_K_deg (T_K))
